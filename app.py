...
# ================================================================
# UNIVERZALNI PREVODILAC (TEKST I SLIKE SA KONTRAST FILTERIMA)
# ================================================================
...
def univerzalni_prevod():
    ...
    # Tekst unet ručno ima apsolutni prioritet nad slikom
    if tekst_za_prevod:
        ...
        try:
            ...
            # Pretvaranje u crno-belu strukturu (uklanjanje šarenila i džempera)
            slika = slika.convert('L')
            
            # Pojačavanje kontrasta za 3.5 puta da obojena slova potamne do maksimuma
            enhancer = ImageEnhance.Contrast(slika)
            slika = enhancer.enhance(3.5)
            
            # Izoštravanje ivica fonta
            sharpener = ImageEnhance.Sharpness(slika)
            slika = sharpener.enhance(2.5)
            
            lang_ocr = 'deu' if izvor == 'de' else 'srp'
            
            # Skeniranje slike sa parametrom psm 11 za nepravilne i razbacane kolone
            finalni_tekst = pytesseract.image_to_string(slika, lang=lang_ocr, config=r'--psm 11').strip()
            
            if not finalni_tekst:
                return jsonify({"error": "Slova na slici nisu prepoznata. Očistite sliku i pokušajte ponovo."})
    ...
    # Slanje pročišćenog teksta na prevođenje
    try:
